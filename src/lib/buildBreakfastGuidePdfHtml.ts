...
export type BuildBreakfastGuidePdfHtmlOptions = {
  /**
   * Carpeta `public/` del repo: lee logo e imágenes desde disco como `data:` URLs.
   * Así Puppeteer no depende de `npm run dev` ni de peticiones HTTP desde `about:blank`.
   */
...
function dataUrlFromPublicFile(publicRoot: string, webPath: string): string {
  const rel = webPath.startsWith("/") ? webPath.slice(1) : webPath;
  const abs = join(publicRoot, rel);
  const buf = readFileSync(abs);
  const mime = mimeForExt(abs);
 ...
 * Con `localPublicRoot`, logo e imágenes van incrustadas desde `public/` (recomendado al generar el PDF en local).
 * Sin eso, usa `baseUrl` absoluto (p. ej. sitio en marcha) para resolver `/guide/...` por HTTP.
 */
export function buildBreakfastGuidePdfHtml(
  baseUrl: string,
  options?: BuildBreakfastGuidePdfHtmlOptions,
): string {
  const publicRoot = options?.localPublicRoot;

  function assetSrc(webPath: string): string {
    if (publicRoot) {
      return dataUrlFromPublicFile(publicRoot, webPath);
    }
    return absUrl(baseUrl, webPath);
  }

  const logoSrc = assetSrc("/ibsa-logo-white.svg");
  ...
  const recipesHtml = recipes
    .map((recipe) => {
      const imgSrc = assetSrc(recipe.imageUrl);
